// AddEditCar
// - Purpose: Form UI for adding or editing a car. Includes inputs for model, licenseplate, year, price, description and an image picker.
// - Props:
//    - editingId: id of car being edited (optional)
//    - model, setModel: state and setter for model input
//    - licenseplate, setLicensePlate: state and setter for license plate input
//    - year, setYear: state and setter for year input
...
export default function AddEditCar({
  ...
  return (
    <View>
      ...
      <TextInput style={styles.input} placeholder="LicensePlate" value={licensePlate} onChangeText={setLicensePlate} />
      <TextInput style={styles.input} placeholder="Year (optional)" value={year} onChangeText={setYear} keyboardType="numeric" />
      ...
      <View style={styles.buttonsRow}>
        <Button
          title={posting ? (editingId ? 'Saving...' : 'Adding...') : (editingId ? 'Save' : 'Add Car')}
          onPress={onSave}
          disabled={posting}
        />
