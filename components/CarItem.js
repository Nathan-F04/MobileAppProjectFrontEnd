//CarItem
// - Purpose: Renders a single car entry in the car list, including image and action buttons.
// - Props:
//    - item: car object (expects fields like _id/id, model,licensePlate,year, price, description, image)
...
export default function CarItem({ item, onEdit, onDelete, onAdd }) {
  ...
  // Renders car fields if present and shows action buttons.
  return (
    <View style={styles.item}>
      <Text style={styles.itemTitle}>{item.model || 'Unnamed'}</Text>
      {item.licensePlate !== undefined && item.licensePlate !== null ? (
        <Text style={styles.itemLicensePlate}>{`${item.licensePlate}`}</Text>
      ) : null}
...
const styles = StyleSheet.create({
  ...
  itemLicensePlate: { color: '#333', marginTop: 4 },
  itemYear: { color: '#333', marginTop: 4 },
