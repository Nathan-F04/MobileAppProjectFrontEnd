...
export default function CarBasket({ route }) {
  ...
  return (
    <View style={styles.container}>
      ...
      {loading ? <ActivityIndicator size="large" /> : (
        <FlatList
          data={basket.items || []}
          keyExtractor={(i, idx) => i._id || i.car?._id || i.car?.id || String(idx)}
          renderItem={({ item }) => (
            <View style={styles.item}>
              <Text style={styles.itemTitle}>{item.car?.model || 'Deleted car'}</Text>
              <Text>€{item.car?.price ?? 0} x {item.quantity}</Text>
